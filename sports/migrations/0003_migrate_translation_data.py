...
def migrate_translation_data(apps, schema_editor):
    ...
    InfrastructureCategoryTranslation = apps.get_model('sports', 'InfrastructureCategoryTranslation')
    InfrastructureObject = apps.get_model('sports', 'InfrastructureObject')
    InfrastructureObjectTranslation = apps.get_model('sports', 'InfrastructureObjectTranslation')
    ...
    for infrastructure in Infrastructure.objects.all():
        translations = InfrastructureTranslation.objects.filter(infrastructure=infrastructure)
        for trans in translations:
            if trans.language == 'ru':
    ...
    for category in InfrastructureCategory.objects.all():
        translations = InfrastructureCategoryTranslation.objects.filter(category=category)
        for trans in translations:
            if trans.language == 'ru':
    ...
    for obj in InfrastructureObject.objects.all():
        translations = InfrastructureObjectTranslation.objects.filter(infrastructure_object=obj)
        for trans in translations:
            if trans.language == 'ru':
...
def reverse_migration(apps, schema_editor):
    ...
    Infrastructure.objects.all().update(
        name_ru='', name_kg='', name_en='',
        description_ru='', description_kg='', description_en='',
        stat_1_label_ru='', stat_1_label_kg='', stat_1_label_en='',
        stat_2_label_ru='', stat_2_label_kg='', stat_2_label_en='',
        stat_3_label_ru='', stat_3_label_kg='', stat_3_label_en='',
        stat_4_label_ru='', stat_4_label_kg='', stat_4_label_en=''
    )
    InfrastructureCategory.objects.all().update(
        name_ru='', name_kg='', name_en=''
    )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ('sports', '0002_alter_infrastructurecategorytranslation_unique_together_and_more'),
    ]
