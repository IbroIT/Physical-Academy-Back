...
class QuotaTypeViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return QuotaType.objects.filter(is_active=True).prefetch_related(
...
class BachelorQuotasViewSet(viewsets.GenericViewSet):
    ...
    def list(self, request):
        ...
        serializer = BachelorQuotasDataSerializer(
            {}, 
...
class DoctorProgramsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if program_id:
            ...
            serializer = DoctorProgramsFullSerializer(program, context={'language': language})
            return Response(serializer.data)
        else:
            # Список всех программ с короткой информацией
            programs = DoctorPrograms.objects.all()
            serializer = DoctorProgramsShortSerializer(programs, many=True, context={'language': language})
            return Response(serializer.data)
...
class CollegeProgramsViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        if program_id:
            ...
            serializer = CollegeProgramsFullSerializer(program, context={'language': language})
            return Response(serializer.data)
        else:
            # Список всех программ с короткой информацией
            programs = CollegePrograms.objects.all()
            serializer = CollegeProgramsShortSerializer(programs, many=True, context={'language': language})
            return Response(serializer.data)
